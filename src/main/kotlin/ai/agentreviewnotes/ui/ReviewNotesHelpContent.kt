...
internal object ReviewNotesHelpContent {
    val text = """
        ...
        Open note details by double-clicking a note or pressing Enter. Select a note and press F4 to open its target. Click the compact note text shown above its anchor to open details. Use the one Edit button to change Type, Status, and Note together, plus Tags and Depends on, regardless of its current status; Save or Cancel without opening another dialog. Dependencies must form an acyclic graph. Delete remains confirmed and is unavailable while another note depends on the selected note. Press Ctrl+Alt+Shift+R while the caret is on a noted line or a selection intersects a note; overlapping notes are offered in a chooser. The selected code range is highlighted in the editor with type-specific colors, and files and directories with visible notes receive badges in Project view.
