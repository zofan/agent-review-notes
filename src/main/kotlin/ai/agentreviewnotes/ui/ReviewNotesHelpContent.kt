...
internal object ReviewNotesHelpContent {
    val text = """
        ...
        Open note details by double-clicking a note or pressing Enter. Select a note and press F4 to open its target. Click the compact note text shown above its anchor to open details. Edit Type and Status inline with their Edit buttons. Edit Note with its Edit button or by double-clicking the note text; Save or Cancel without opening another dialog. Delete remains a confirmed destructive action. Press Ctrl+Alt+Shift+R while the caret is on a noted line or a selection intersects a note; overlapping notes are offered in a chooser. The selected code range is highlighted in the editor with type-specific colors, and files and directories with visible notes receive badges in Project view.
