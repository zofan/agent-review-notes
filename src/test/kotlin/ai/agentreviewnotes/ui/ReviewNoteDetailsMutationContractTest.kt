...
class ReviewNoteDetailsMutationContractTest {
    ...
    fun `one edit action switches the complete details form into edit mode`() {
        ...
        assertContains(source, "onSave: (ReviewKind, ReviewStatus, String, List<String>, List<String>) -> CompletableFuture<*>")
        assertContains(source, "Tags:")
        assertContains(source, "Depends on:")
    ...
    fun `details service exposes canonical futures to inline controls`() {
        ...
        assertContains(source, "onSave = { kind, status, message, tags, dependsOn ->")
        assertContains(source, "store.updateAsync(note, kind, status, message, tags, dependsOn)")
